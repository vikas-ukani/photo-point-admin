                ...
                >
                    ...
                    <tbody v-if="lists && lists.length">
                        <tr v-for="(list, index) in lists" :key="index">
                            ...
                            <td>
                                <small class="font-bold">{{ (list.size_number )|| '-' }}</small>
                            </td>
                            <td>
        ...
        >
            ...
            >
                ...
                <div class="input-group mb-3 col-md-6">
                    ...
                    <small
                        v-if="errors.has('size')"
                        class="text-danger mt-1"
                    >{{ errors.first('size') }}</small>
                </div>

                <div class="input-group mb-3 col-md-6">
                    <label for="size_number" class="text-capitalize ml-3">
                        size number
                        <small
                            :class="!detail.size_number || errors.has('size_number') ? 'text-danger' : 'text-success' "
                        >*</small>
                    </label>
                    <div class="input-group">
                        <input
                            ...
                            v-model="detail.size_number"
                            v-validate="'required'"
                            :class="{ 'is-invalid': errors.has('size_number') }"
                        />
                    </div>
                    <small
                        v-if="errors.has('size_number')"
                        class="text-danger mt-1"
                    >{{ errors.first('size_number') }}</small>
                </div>
...
export default {
    ...
    methods: {
        ...
        /** get details by id */
        async getDetails(id) {
            let res = await Services.call(ApiCollections.products_get).getOne(
                id
            );
        ...
        async deleteRecord(list) {
            ...
            let res = await Services.call(
                ApiCollections.products_delete
            ).delete(list.id);
            if (res && res.success && res.success == true) {
        ...
        async submitData() {
            ...
            formData.append("size_number", this.detail.size_number);
            ...
            if (this.detail && this.detail.id) {
                ...
                /** create data */
                let res = await Services.call(
                    ApiCollections.products_create
                ).post(formData);
                /** set data  */
                if (res && res.success && res.success == true) {
