  ...
  <div class="animated fadeIn">
    <div class>
      ...
      <div class="card card-body" v-if="lists && lists.length">
        <table
          class="table table-hover table-responsive table-responsive-sm col-lg-12 col-md-12 col-sm-12 col-xs-12"
        >
          <thead>
            <tr>
              <th nowrap>
              ...
              <!-- <th class="text-capitalize">is_added_to_shiprocket</th> -->
          ...
          <tbody v-if="lists &&  lists.length">
            <tr v-for="(list, index) in lists" :key="index">
              ...
              <td class="font-bold">{{ (list.pickup_location || '-') }}</td>
              <td class="font-bold">{{ (list.name || '-') }}</td>
              <td class="font-bold">{{ (list.email || '-') }}</td>
              <td class="font-bold">{{ (list.phone || '-') }}</td>
              <td class="font-bold">{{ (list.address || '-') }}</td>
              <td class="font-bold">{{ (list.address_2 || '-') }}</td>
              <td class="font-bold">{{ (list.city || '-') }}</td>
              <td class="font-bold">{{ (list.state || '-') }}</td>
              <td class="font-bold">{{ (list.country || '-') }}</td>
              <td class="font-bold">{{ (list.pin_code || '-') }}</td>
              <td>
                <span
                  class="p-0 m-0"
                  @click="statusChange('is_added_to_shiprocket', !list.is_added_to_shiprocket, list.id)"
                >
                  <switches
                    v-model="list.is_added_to_shiprocket"
                    v-bind:label="(!!list.is_added_to_shiprocket) ? 'Approve' : 'Deactive'"
                    theme="bulma"
                    color="green"
                    type-bold="false"
                  ></switches>
...
export default {
  ...
  methods: {
    ...
    async getAllList(search = "") {
      ...
      /** start progress here */
      this.$Progress.start();
      let res = await Services.call(ApiCollections.pickup_address_listing).post(
        request
      );
    ...
    async statusChange(key, value, id) {
      ...
      let res = await Services.call(
        ApiCollections.pickup_address_approve_status
      ).post(request);
